...
final class ListModesTest: XCTestCase {
    ...
    func testListModesOutput() async throws {
        config.modes = [
            "main": Mode(bindings: [:], tapBindings: [:]),
            "service": Mode(bindings: [:], tapBindings: [:]),
            "resize": Mode(bindings: [:], tapBindings: [:]),
        ]
        ...
        let expectedJson = JSONEncoder.winMuxDefault.encodeToString([
            ["mode-id": "main"],
            ["mode-id": "resize"],
            ["mode-id": "service"],
        ])
        ...
        let expectedCurrentJson = JSONEncoder.winMuxDefault.encodeToString([
            ["mode-id": "main"],
        ])
